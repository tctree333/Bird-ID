...
class TestScore:
    ...
    def test_score_none(self):
        ...
        coroutine = self.cog.score.callback(
            self.cog, self.ctx
        )  # pylint: disable=no-member
        assert asyncio.run(coroutine) is None
        assert (
            self.ctx.messages[2].content
            == "Wow, looks like a total of `0` birds have been answered correctly in this **channel**!\nGood job everyone!"
        )
    ...
    def test_with_score(self):
        ...
        assert (
            self.ctx.messages[2].content
            == "Wow, looks like a total of `20` birds have been answered correctly in this **channel**!\nGood job everyone!"
        )
    ...
    def test_userscore_self(self):
        ...
        coroutine = self.cog.userscore.callback(
            self.cog, self.ctx
        )  # pylint: disable=no-member
        assert asyncio.run(coroutine) is None
        ...
        assert (
            self.ctx.messages[2].embed.fields[0].value
            == f"<@{self.ctx.author.id}> has answered correctly 0 times."
        )
    ...
    def test_userscore_bad_input(self):
        ...
        coroutine = self.cog.userscore.callback(
            self.cog, self.ctx, user=test_input
        )  # pylint: disable=no-member
        assert asyncio.run(coroutine) is None
    ...
    def test_userscore_user_none(self):
        ...
        coroutine = self.cog.userscore.callback(
            self.cog, self.ctx, user=self.ctx.guild.members[0]
        )  # pylint: disable=no-member
        assert asyncio.run(coroutine) is None
        assert (
            self.ctx.messages[2].content == "This user does not exist on our records!"
        )
